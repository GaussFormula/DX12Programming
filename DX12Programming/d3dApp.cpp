...
void D3DApp::CreateCommandObjects()
{
    ...
    ThrowIfFailed(md3dDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, 
        IID_PPV_ARGS(&mDirectCmdListAllocator)));
...
void D3DApp::FlushCommandQueue()
{
    ...
    if (mFence->GetCompletedValue() < mCurrentFence)
    {
        ...
        // Fire event when GPU hits current fence event is fired.
...
void D3DApp::OnResize()
{
    ...
    CD3DX12_CPU_DESCRIPTOR_HANDLE rthHeapHandle(mRtvHeap->GetCPUDescriptorHandleForHeapStart());
    for (UINT i = 0; i < SwapChainBufferCount; ++i)
    {
        ThrowIfFailed(mSwapChain->GetBuffer(i, IID_PPV_ARGS(&mSwapChainBuffer[i])));
        md3dDevice->CreateRenderTargetView(mSwapChainBuffer[i].Get(), nullptr, rthHeapHandle);
        rthHeapHandle.Offset(1, mRtvDescriptorSize);
    }
